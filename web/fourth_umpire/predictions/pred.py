...
def predict_if_bat_win(team_batting,team_bowling,run,ball,wicket,target,city):
    rel_path = "2st_inn/bat_win/2st_inn_bat_win_wicket.pkl"
    abs_file_path = os.path.join(script_dir, rel_path)
    regressor = joblib.load(abs_file_path)

    rel_path = "2st_inn/bat_win/2st_inn_bat_win_hot.pkl"
    abs_file_path = os.path.join(script_dir, rel_path)
    enc = pickle.load( open( abs_file_path, "rb" ))
...
def predict_if_bowl_win(team_batting,team_bowling,run,ball,wicket,target,city):
    rel_path = "2st_inn/bowl_win/2st_inn_bowl_win_run.pkl"
    abs_file_path = os.path.join(script_dir, rel_path)
    regressor = joblib.load(abs_file_path)

    rel_path = "2st_inn/bowl_win/2st_inn_bowl_win_hot.pkl"
    abs_file_path = os.path.join(script_dir, rel_path)
    enc = pickle.load( open( abs_file_path, "rb" ))
...
def predict_2nd_end_ball(team_batting,team_bowling,run,ball,wicket,target,city):
    rel_path = "2st_inn/end/2st_inn_end.pkl"
    abs_file_path = os.path.join(script_dir, rel_path)
    regressor = joblib.load(abs_file_path)

    rel_path = "2st_inn/end/2st_inn_end_hot.pkl"
    abs_file_path = os.path.join(script_dir, rel_path)
    enc = pickle.load( open( abs_file_path, "rb" ))
    ...
    X_test = enc.transform(X_test).toarray()
    y_pred = regressor.predict(X_test)
    print("our_prediction:2st_inn_end",y_pred)
    return y_pred[0]



def predict_2nd_inn(team_batting,team_bowling,run,ball,wicket,target,city):
    rel_path = "2st_inn/who_win/2st_inn_win_pred.pkl"
    abs_file_path = os.path.join(script_dir, rel_path)
    regressor = joblib.load(abs_file_path)

    rel_path = "2st_inn/who_win/2st_inn_win_hot.pkl"
    abs_file_path = os.path.join(script_dir, rel_path)
    enc = pickle.load( open( abs_file_path, "rb" ))
